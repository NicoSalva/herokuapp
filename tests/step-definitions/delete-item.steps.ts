...
When('I create a new temporary item', async () => {
  ...
  const incResult = await QAUtils.waitUntilCondition(
    async () => {
    ...
    },
    {
      timeout: 30000,
      interval: 1000,
      timeoutMsg: `Item count did not increase from ${(global as any).initialItemCount} after creating temp item`,
    }
  );
...
When('I delete the item with text {string}', async (itemText: string) => {
  ...
  await expect(existsBefore).toBe(true);
...
Then('I should not see the item with text {string}', async (itemText: string) => {
  ...
  await QAUtils.waitUntilCondition(
    async () => {
    ...
    },
    {
      timeout: 15000,
      interval: 1000,
      timeoutMsg: `Item "${itemText}" did not disappear after delete`,
    }
  );

  const existsAfter = await listPage.isItemInList(itemText);
  await expect(existsAfter).toBe(false);
